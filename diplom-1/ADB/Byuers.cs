       ...
       MessageBoxIcon.Information);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                DB db = new DB();
                db.openConnection();
                MySqlCommand command = new MySqlCommand("Update avtomobiles,buyers set av_status = 'В наличии' where b_id = @id ", db.getConnection());
                ...
                command = new MySqlCommand("DELETE FROM buyers where b_id = @id", db.getConnection());
                command.Parameters.Add("@id", MySqlDbType.VarChar).Value = dataGridView1.CurrentRow.Cells["id"].Value.ToString();//сравнение айди со строкой
                command.ExecuteNonQuery();
        ...
        private void comboBox1_Enter(object sender, EventArgs e)
        {
            ...
            DataTable table = new DataTable();
            MySqlDataAdapter adapter = new MySqlDataAdapter();
            MySqlCommand command = new MySqlCommand("Select av_vin from avtomobiles where av_status = 'В наличии' ", db.getConnection());
            adapter.SelectCommand = command;
            adapter.Fill(table);
            comboBox1.Items.Clear();
            for (int i = 0; i < table.Rows.Count; i++)
            {
                comboBox1.Items.Add(table.Rows[i]["av_vin"]);
            }
        }
        ...
        private void updavto()
        {
            ...
            DataTable table = new DataTable();
            MySqlDataAdapter adapter = new MySqlDataAdapter();
            MySqlCommand command = new MySqlCommand("update avtomobiles, buyers set av_status = 'Забронировано' Where av_vin = b_cid", db.getConnection());
            adapter.SelectCommand = command;
            adapter.Fill(table);
        }
    }
